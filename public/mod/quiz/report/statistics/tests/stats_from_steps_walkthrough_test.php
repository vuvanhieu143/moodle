...
final class stats_from_steps_walkthrough_test extends \mod_quiz\tests\attempt_walkthrough_testcase {
    ...
    /**
     * Test structure analysis when multiple random slots select questions from the same category across versions.
     *
     * @covers \quiz_statistics_report::get_all_stats_and_analysis
     * @covers \core_question\statistics\questions\all_calculated_for_qubaid_condition::structure_analysis_for_one_slot
     */
    public function test_two_random_slots_from_same_category_with_multiple_versions(): void {
        $this->resetAfterTest();
        $this->setAdminUser();
        $this->course = $this->getDataGenerator()->create_course();
        $coursecontext = \context_course::instance($this->course->id);
        ...
        $questiongenerator = $this->getDataGenerator()->get_plugin_generator('core_question');
        $cat = $questiongenerator->create_question_category(['context' => $coursecontext->id]);
        $q1 = $questiongenerator->create_question('multichoice', null, ['category' => $cat->id, 'name' => 'Q1']);
        $q2 = $questiongenerator->create_question('multichoice', null, ['category' => $cat->id, 'name' => 'Q2']);
        $q3 = $questiongenerator->create_question('multichoice', null, ['category' => $cat->id, 'name' => 'Q3']);
        ...
        $this->add_random_questions($quiz->id, 2, $cat->id, 1);

        $user1 = $this->getDataGenerator()->create_user();
        $user2 = $this->getDataGenerator()->create_user();
        $user3 = $this->getDataGenerator()->create_user();

        // First set of attempts before version updates.
        $this->setUser($user1);
        $a1 = $quizgenerator->create_attempt($quiz->id, $user1->id);
        $quizgenerator->submit_responses($a1->id, [1 => 'True', 2 => 'True'], false, true);

        $this->setUser($user2);
        $a2 = $quizgenerator->create_attempt($quiz->id, $user2->id);
        $quizgenerator->submit_responses($a2->id, [1 => 'True', 2 => 'True'], false, true);

        $this->setUser($user3);
        $a3 = $quizgenerator->create_attempt($quiz->id, $user3->id);
        $quizgenerator->submit_responses($a3->id, [1 => 'True', 2 => 'True'], false, true);

        // Update question versions in the category.
        $q1v2 = $questiongenerator->update_question($q1);
        $q2v2 = $questiongenerator->update_question($q2);
        $q3v2 = $questiongenerator->update_question($q3);

        // Second set of attempts after question versions were updated.
        $this->setUser($user1);
        $a4 = $quizgenerator->create_attempt($quiz->id, $user1->id);
        $quizgenerator->submit_responses($a4->id, [1 => 'True', 2 => 'True'], false, true);

        $this->setUser($user2);
        $a5 = $quizgenerator->create_attempt($quiz->id, $user2->id);
        $quizgenerator->submit_responses($a5->id, [1 => 'True', 2 => 'True'], false, true);

        $this->setUser($user3);
        $a6 = $quizgenerator->create_attempt($quiz->id, $user3->id);
        $quizgenerator->submit_responses($a6->id, [1 => 'True', 2 => 'True'], false, true);
        ...
        $reporturl = new \moodle_url('/mod/quiz/report.php', ['id' => 1, 'mode' => 'statistics']);
        $table->statistics_setup($quiz, 1, $reporturl, 6);

        // Verify main quiz structure analysis table rows.
        $mainrows = [];
        foreach ($questionstats->get_all_slots() as $slot) {
            $analysis = $questionstats->structure_analysis_for_one_slot($slot, true);
            foreach ($analysis as $row) {
                $formatted = $table->format_row($row);
                $mainrows[] = [
                    'slot' => $slot,
                    'class' => get_class($row),
        ...
        // Both Slot 1 and Slot 2 must have their random question row and summary row linking to their respective slot.
        $this->assertCount(4, $mainrows);

        $this->assertEquals(1, $mainrows[0]['number']);
        $this->assertStringContainsString('Random question', $mainrows[0]['name']);

        $this->assertEquals('', $mainrows[1]['number']);
        $this->assertStringContainsString('Range of statistics for these questions', $mainrows[1]['name']);
        $this->assertStringContainsString('slot=1', $mainrows[1]['name']);

        $this->assertEquals(2, $mainrows[2]['number']);
        $this->assertStringContainsString('Random question', $mainrows[2]['name']);

        $this->assertEquals('', $mainrows[3]['number']);
        $this->assertStringContainsString('Range of statistics for these questions', $mainrows[3]['name']);
        $this->assertStringContainsString('slot=2', $mainrows[3]['name']);

        // Verify detailed structure analysis for Slot 1.
        $slot1analysis = $questionstats->structure_analysis_for_one_slot(1, false);
        $this->assertGreaterThan(1, count($slot1analysis));
        $slot1formatted = array_map([$table, 'format_row'], $slot1analysis);
        $this->assertEquals(1, $slot1formatted[0]['number']);
        for ($i = 1; $i < count($slot1formatted); $i++) {
            $this->assertMatchesRegularExpression('/^1\.\d+$/', (string) $slot1formatted[$i]['number']);
            $this->assertStringContainsString('qid=', $slot1formatted[$i]['name']);
        }

        // Verify detailed structure analysis for Slot 2.
        $slot2analysis = $questionstats->structure_analysis_for_one_slot(2, false);
        $this->assertGreaterThan(1, count($slot2analysis));
        $slot2formatted = array_map([$table, 'format_row'], $slot2analysis);
        $this->assertEquals(2, $slot2formatted[0]['number']);
        for ($i = 1; $i < count($slot2formatted); $i++) {
            $this->assertMatchesRegularExpression('/^2\.\d+$/', (string) $slot2formatted[$i]['number']);
            $this->assertStringContainsString('qid=', $slot2formatted[$i]['name']);
        }
    }
}
